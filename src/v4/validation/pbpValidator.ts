 ...
 */
export function pbpValidator(
  options: PBPValidationOptions,
): PBPValidationResult {
  const {
    points,
    expectedScore,
  ...
  const sets = finalScore.sets.map((set) => {
    ...
    const tiebreak: [number, number] | undefined =
      set.side1TiebreakScore === undefined
        ? undefined
        : [set.side1TiebreakScore, set.side2TiebreakScore || 0];

    let scoreString = `${games[0]}-${games[1]}`;
    if (tiebreak) {
      scoreString =
        games[0] > games[1]
          ? `${games[0]}-${games[1]}(${tiebreak[1]})`
          : `${games[0]}(${tiebreak[0]})-${games[1]}`;
    }
