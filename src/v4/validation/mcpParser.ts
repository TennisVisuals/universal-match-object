 ...
 */
export function parsePointWinner(
  mcpPoint: MCPPoint,
  serverIndex: 0 | 1,
): 0 | 1 {
...
export function shotSplitter(shotSequence: string): string[] {
  ...
  for (let i = 0; i < shotSequence.length; i++) {
    ...
    // Check if this is a stroke code (starts new shot)
    ...
    // Check if this is a serve code (starts new shot)
    else if (serveCodes.includes(char)) {
      // Push previous shot if exists
      if (currentShot) {
        shots.push(currentShot);
      }
...
export function analyzeSequence(shotSequence: string): ShotParseResult {
  ...
  if (ignoredShots && ignoredShots.length) {
    // Store but don't use
  }
...
export function shotParser(shotSequence: string, whichServe: 1 | 2): ShotParseResult {
  ...
  if (!parsedShots.terminator) {
    if (
      !shotFault(parsedShots.serves[0]) &&
      parsedShots.serves.length > 2
    ) {
      parsedShots.winner = "R";
      return parsedShots;
    }
  ...
  const lastPlayer: "S" | "R" =
    (parsedShots.serves.length + parsedShots.rally.length) % 2 === 0
      ? "R"
      : "S";
  const finalShot =
    parsedShots.rally.length > 0
      ? parsedShots.rally[parsedShots.rally.length - 1]
      : parsedShots.serves.length > 0
        ? parsedShots.serves[parsedShots.serves.length - 1]
        : undefined;
...
export function parseMCPPoint(
  ...
  const rallySequence = buildRallySequence(
    parsed.serves,
    parsed.rally,
    serverIndex,
  );
