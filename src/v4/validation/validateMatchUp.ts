...
import type { MatchUp, SetScore, Participant } from "../types";
...
export function validateMatchUp(
  options: ValidateMatchUpOptions,
): ValidationDetails {
  ...
  for (const point of points) {
    try {
      ...
      errors.push(
        `Point ${point.pointNumber || pointsProcessed + 1}: ${errorMessage}`,
      );
    }
  }
  ...
  if (expectedScore) {
    ...
    if (expectedScore.sets) {
      if (actualScore.sets.length !== expectedScore.sets.length) {
        errors.push(
          `Set count mismatch: expected ${expectedScore.sets.length}, got ${actualScore.sets.length}`,
        );
      }
      ...
      expectedScore.sets.forEach((expectedSet, index) => {
        ...
        // Compare game scores
        if (
          expectedSet.side1Score !== undefined &&
          actualSet.side1Score !== expectedSet.side1Score
        ) {
          errors.push(
            `Set ${index + 1} side1Score: expected ${expectedSet.side1Score}, got ${actualSet.side1Score}`,
          );
        }

        if (
          expectedSet.side2Score !== undefined &&
          actualSet.side2Score !== expectedSet.side2Score
        ) {
          errors.push(
            `Set ${index + 1} side2Score: expected ${expectedSet.side2Score}, got ${actualSet.side2Score}`,
          );
        }
    ...
    if (expectedScore.scoreString) {
      if (actualScore.scoreString !== expectedScore.scoreString) {
        errors.push(
          `Score string mismatch: expected "${expectedScore.scoreString}", got "${actualScore.scoreString}"`,
        );
      }
    }
  }
