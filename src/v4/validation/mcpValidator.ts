...
export function validateMCPMatch(
  ...
  for (let i = 0; i < mcpMatch.points.length; i++) {
    ...
    const serverFromData = mcpPoint.Svr === "1" ? 0 : 1;
    currentServer = serverFromData as 0 | 1;
    ...
    try {
      ...
      if (debug && (i < 5 || i === mcpMatch.points.length - 1)) {
        ...
        console.log(
          `Point ${i + 1}: ${parsedPoint.result || "Rally"} → ${score.scoreString}`,
        );
      }
    } catch (error) {
      const errorMessage =
  ...
  if (validateScore && expectedScore) {
    ...
    if (!scoreMatches) {
      errors.push(
        `Score mismatch: expected "${expectedScore}", got "${actualScore}"`,
      );
    }
...
export function mcpValidator(
  ...
  const matchesToProcess = matchId
    ? matches.filter((m) => m.match_id === matchId)
    : matches;

  if (matchesToProcess.length === 0) {
    errors.push(
      matchId
        ? `Match ID not found: ${matchId}`
        : "No matches found in CSV data",
    );
    return {
      valid: false,
      errors,
