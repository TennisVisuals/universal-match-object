 ...
 * 
 * Calculate statistics without PointsEngine or v3 adapter.
 ...
 * the PointsEngine or v3 adapter.
 * 
 ...
 * ```
 */
export function enrichPointHistory(
  rawPoints: any[],
  matchUp: MatchUp
): PointWithMetadata[] {
...
export function getQuickStats(
  ...
  return {
    aces: [
      counters.teams[0].aces?.length || 0,
      counters.teams[1].aces?.length || 0,
    ],
    doubleFaults: [
      counters.teams[0].doubleFaults?.length || 0,
      counters.teams[1].doubleFaults?.length || 0,
    ],
    winners: [
      counters.teams[0].winners?.length || 0,
      counters.teams[1].winners?.length || 0,
    ],
    unforcedErrors: [
      counters.teams[0].unforcedErrors?.length || 0,
      counters.teams[1].unforcedErrors?.length || 0,
    ],
    totalPoints: [
      counters.teams[0].pointsWon?.length || 0,
      counters.teams[1].pointsWon?.length || 0,
    ],
  };
}
