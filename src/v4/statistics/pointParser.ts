...
export function categorizePoint(point: PointWithMetadata): string[] {
  ...
  // Serve tracking
  if (point.winner === point.server || (!point.winner && point.code && SERVER_WINS_CODES.includes(point.code))) {
    // Server served this point
    categories.push('pointsServed');
    if (point.serve === 1) {
      categories.push('serves1stIn'); // Made it in play
    } else if (point.serve === 2) {
      categories.push('serves2ndIn');
    }
  }
