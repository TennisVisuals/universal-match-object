...
describe("Player Round-Trip Tests", () => {
  ...
  it("should export and import players correctly", () => {
    ...
    if (exported.sides?.[0]?.participant) {
      ...
      expect(exported.sides[0].participant.person?.standardGivenName).toBe(
        "Player",
      );
      expect(exported.sides[0].participant.person?.standardFamilyName).toBe(
        "One",
      );
    }
    ...
    if (exported.sides?.[1]?.participant) {
      ...
      expect(exported.sides[1].participant.person?.standardGivenName).toBe(
        "Player",
      );
      expect(exported.sides[1].participant.person?.standardFamilyName).toBe(
        "Two",
      );
    }

    // Import back
    const imported = umo.fromMatchUp(exported);
